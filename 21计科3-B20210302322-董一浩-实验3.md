...
def solution(number)
    ...
    multi3tor5 = [i for i in range(number) if i%3 == 0 or i%5 == 0]
    return sum(multi3tor5)
```
...
#### 第二题： 1重复字符的编码器（Duplicate Encoder）
...
#### 第四题： 1从随机三元组中恢复秘密字符串(Recover a secret string from random triplets)
...
```mermaid
flowchart LR
    A[letters] --> B{Is it in the aeiou?}
    B -->|yes| C[clean]
    B ---->|No| E[print]
```
